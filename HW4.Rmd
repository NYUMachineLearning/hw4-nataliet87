...
```
^^ larger change here indicates greater importance of the feature in classificiation
...
**Part I:**
Compare the most important features from at least 2 different classes of feature selection methods covered in this tutorial with any reasonable machine learning dataset from mlbench. Do these feature selection methods provide similar results? 
...
y <- as.double(as.matrix(ifelse(PimaIndiansDiabetes[,9]=='pos', 0, 1))) 
...
#fit a model
rfmodel = randomForest(diabetes ~ glucose + pregnant + pressure + triceps + insulin + mass + pedigree + age,
                       data=train_classification,  importance = TRUE, oob.times = 15, confusion = TRUE)
...
These two approaches generated slightly different results, but not substantially so. And re-running the embedded feature selection would somewhat different important features. Both selected glucose and pregnancy as the most important two features (though in two different orders, and with differing levels of importance attributed), but later important features were ordered differently. 



**Part II:**
Attempt a feature selection method not covered in this tutorial (backward elimination, forward propogation, etc.)

Used wrapper method: feature selection by simluated annealing. This approach tests the model on 
random subsets of features to determine the best features on which to build the model
```{r}
ctrl <- safsControl(functions = treebagSA)
```

```{r}
obj <- safs(x = PimaIndiansDiabetes[,1:8],
